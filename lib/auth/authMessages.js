...
function generateTransitKey() {
    const transitKey = index_1.makeECPrivateKey();
    return transitKey;
}
...
function makeAuthRequestImpl(transitPrivateKey, redirectURI, manifestURI, scopes, appDomain = window.location.origin, expiresAt, extraParams = {}) {
    ...
    const address = index_1.publicKeyToAddress(publicKey);
 ...
 * @private
 */
function makeAuthResponse(privateKey, profile = {}, username = null, metadata, coreToken = null, appPrivateKey = null, expiresAt = index_1.nextMonth().getTime(), transitPublicKey = null, hubUrl = null, blockstackAPIUrl = null, associationToken = null) {
    /* Convert the private key to a public key to an issuer */
