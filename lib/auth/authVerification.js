...
function doPublicKeysMatchIssuer(token) {
    const payload = jsontokens_1.decodeToken(token).payload;
    const publicKeys = payload.public_keys;
    const addressFromIssuer = index_1.getAddressFromDID(payload.iss);
    if (publicKeys.length === 1) {
        const addressFromPublicKeys = index_1.publicKeyToAddress(publicKeys[0]);
...
function isManifestUriValid(token) {
    const payload = jsontokens_1.decodeToken(token).payload;
    return index_1.isSameOriginAbsoluteUrl(payload.domain_name, payload.manifest_uri);
}
...
function isRedirectUriValid(token) {
    const payload = jsontokens_1.decodeToken(token).payload;
    return index_1.isSameOriginAbsoluteUrl(payload.domain_name, payload.redirect_uri);
}
...
function verifyAuthRequestAndLoadManifest(token) {
    return Promise.resolve().then(() => verifyAuthRequest(token)
        .then((valid) => {
        if (valid) {
            return _1.fetchAppManifest(token);
        }
        else {
